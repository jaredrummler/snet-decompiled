...
public final class CodedOutputByteBufferNano {
    ...
    private static int encodedLengthGeneral(CharSequence sequence, int start) {
        ...
        while (i < utf16Length) {
            ...
            if (c < '\u0800') {
                ...
                utf8Length += 2;
                if ('\ud800' <= c && c <= '\udfff') {
                    if (Character.codePointAt(sequence, i) < PeopleColumnBitmask.AFFINITY_2) {
    ...
    private static void encodeDirect(CharSequence sequence, ByteBuffer byteBuffer) {
        ...
        while (i < utf16Length) {
            ...
            if (c < '\u0080') {
                ...
                byteBuffer.put((byte) ((c & 63) | LogSource.KEEP));
            } else if (c < '\ud800' || '\udfff' < c) {
                byteBuffer.put((byte) ((c >>> 12) | 480));
                byteBuffer.put((byte) (((c >>> 6) & 63) | LogSource.KEEP));
                byteBuffer.put((byte) ((c & 63) | LogSource.KEEP));
            } else {
                if (i + 1 != sequence.length()) {
                    ...
                    if (Character.isSurrogatePair(c, low)) {
                        ...
                        byteBuffer.put((byte) (((codePoint >>> 12) & 63) | LogSource.KEEP));
                        byteBuffer.put((byte) (((codePoint >>> 6) & 63) | LogSource.KEEP));
                        byteBuffer.put((byte) ((codePoint & 63) | LogSource.KEEP));
                    }
                }
    ...
    private static int encode(CharSequence sequence, byte[] bytes, int offset, int length) {
        ...
        while (i < utf16Length) {
            ...
            if (c < '\u0080' && j2 < limit) {
                ...
                j2 = j + 1;
                bytes[j] = (byte) ((c & 63) | LogSource.KEEP);
                j = j2;
            } else if ((c < '\ud800' || '\udfff' < c) && j2 <= limit - 3) {
                j = j2 + 1;
                bytes[j2] = (byte) ((c >>> 12) | 480);
                j2 = j + 1;
                bytes[j] = (byte) (((c >>> 6) & 63) | LogSource.KEEP);
                j = j2 + 1;
                bytes[j2] = (byte) ((c & 63) | LogSource.KEEP);
            } else if (j2 <= limit - 4) {
                if (i + 1 != sequence.length()) {
                    i++;
                    char low = sequence.charAt(i);
                    if (Character.isSurrogatePair(c, low)) {
                        ...
                        j2 = j + 1;
                        bytes[j] = (byte) (((codePoint >>> 12) & 63) | LogSource.KEEP);
                        j = j2 + 1;
                        bytes[j2] = (byte) (((codePoint >>> 6) & 63) | LogSource.KEEP);
                        j2 = j + 1;
                        bytes[j] = (byte) ((codePoint & 63) | LogSource.KEEP);
                        j = j2;
                    }
                }
                throw new IllegalArgumentException("Unpaired surrogate at index " + (i - 1));
    ...
    public void writeRawVarint32(int value) throws IOException {
        while ((value & -128) != 0) {
            writeRawByte((value & LogSource.TAILORMADE) | LogSource.KEEP);
            value >>>= 7;
        }
        writeRawByte(value);
    }
    ...
    public void writeRawVarint64(long value) throws IOException {
        while ((-128 & value) != 0) {
            writeRawByte((((int) value) & LogSource.TAILORMADE) | LogSource.KEEP);
            value >>>= 7;
        }
