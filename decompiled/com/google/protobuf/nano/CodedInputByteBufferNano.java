...
public final class CodedInputByteBufferNano {
    ...
    public int readRawVarint32() throws IOException {
        ...
        int result = tmp & LogSource.TAILORMADE;
        tmp = readRawByte();
        if (tmp >= null) {
            return result | (tmp << 7);
        }
        result |= (tmp & LogSource.TAILORMADE) << 7;
        tmp = readRawByte();
        if (tmp >= null) {
            return result | (tmp << 14);
        }
        result |= (tmp & LogSource.TAILORMADE) << 14;
        tmp = readRawByte();
        if (tmp >= null) {
            return result | (tmp << 21);
        }
        result |= (tmp & LogSource.TAILORMADE) << 21;
        tmp = readRawByte();
        result |= tmp << 28;
        if (tmp >= null) {
            return result;
    ...
    public long readRawVarint64() throws IOException {
        ...
        for (int shift = 0; shift < DEFAULT_RECURSION_LIMIT; shift += 7) {
            byte b = readRawByte();
            result |= ((long) (b & LogSource.TAILORMADE)) << shift;
            if ((b & LogSource.KEEP) == 0) {
                return result;
            }
        }
        throw InvalidProtocolBufferNanoException.malformedVarint();
    ...
    private CodedInputByteBufferNano(byte[] buffer, int off, int len) {
        this.currentLimit = SimpleClientBuilder.API_PRIORITY_OTHER;
    ...
    public int getBytesUntilLimit() {
        if (this.currentLimit == SimpleClientBuilder.API_PRIORITY_OTHER) {
            return -1;
        }
