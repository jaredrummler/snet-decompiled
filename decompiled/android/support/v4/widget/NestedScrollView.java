...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent, NestedScrollingChild {
    ...
    public boolean executeKeyEvent(KeyEvent event) {
        ...
        if (canScroll()) {
            ...
            if (event.getAction() == 0) {
                switch (event.getKeyCode()) {
                    ...
                    case LogSource.ANDROID_GSA /*20*/:
                        if (!event.isAltPressed()) {
                            handled = arrowScroll(LogSource.CHROMECAST_APP_LOG);
                            break;
                        }
                        handled = fullScroll(LogSource.CHROMECAST_APP_LOG);
                        break;
                    case LogSource.NETSTATS /*62*/:
                        pageScroll(event.isShiftPressed() ? 33 : LogSource.CHROMECAST_APP_LOG);
                        break;
                }
            }
            return handled;
            ...
            View nextFocused = FocusFinder.getInstance().findNextFocus(this, currentFocused, LogSource.CHROMECAST_APP_LOG);
            if (nextFocused == null || nextFocused == this || !nextFocused.requestFocus(LogSource.CHROMECAST_APP_LOG)) {
                return false;
            }
            return true;
        }
    ...
    public boolean pageScroll(int direction) {
        boolean down;
        if (direction == LogSource.CHROMECAST_APP_LOG) {
            down = true;
        } else {
            down = false;
        }
    ...
    public boolean arrowScroll(int direction) {
        ...
        if (nextFocused == null || !isWithinDeltaOfScreen(nextFocused, maxJump, getHeight())) {
            ...
            } else if (direction == LogSource.CHROMECAST_APP_LOG && getChildCount() > 0) {
        ...
        if (currentFocused != null && currentFocused.isFocused() && isOffScreen(currentFocused)) {
            ...
            setDescendantFocusability(PeopleColumnBitmask.AFFINITY_3);
            requestFocus();
    ...
    protected boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
        if (direction == 2) {
            direction = LogSource.CHROMECAST_APP_LOG;
        } else if (direction == 1) {
            direction = 33;
        }
